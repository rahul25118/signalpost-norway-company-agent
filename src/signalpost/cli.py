...
async def run_batch(input_path: str, output_path: str, concurrency: int = 15):
    ...
    async with httpx.AsyncClient() as client:
        async def sem_fetch(org):
            async with sem:
                return await fetch_company_comprehensive(org, client)
    ...
    print(f"Batch completed: {len(results)} profiles saved to {output_path}")

def main():
    parser = argparse.ArgumentParser(description="Signalpost Comprehensive Runner")
    ...
    p_batch.add_argument("--input", required=True, help="Input CSV/txt path")
